...
title <- titlePanel(h4(em("SRAdb Web Application")))
...
queryPanel <- wellPanel(
  fluidRow(
    column(
      4,
      textInput("searchTerms", label = h5("Search Terms:"), value = "")
    ),
    column(
      3,
      selectInput(
...
tabPanel_searchResults <- tabPanel(
  "Search Results",
          ...
          fluidRow(
            ...
            column(
              ...
              selectizeInput(
                ...
                label = NULL,
                choices = list(
                  "Get All Related Accession Codes" = "related_acc",
            ...
            column(
              2,
              conditionalPanel(
                condition = "input.operationType != 'download'",
            ...
            column(
              ...
              downloadButton(
                "downloadFullSRA",
                label = " Export Full SRA Table"
              )
            )
          ),
          conditionalPanel(
            condition = "input.operationType == 'fastqdump'",
            wellPanel(
              fluidRow(
                column(
                  2,
                ...
                column(
                  ...
                  column(
                    ...
                    radioButtons(
                      ...
                      label = NULL,
                      choices = c(
                        "Get Entire File" = "entire",
                        "Custom Range" = "range"
                      )
                    ),
                    conditionalPanel(
                      condition = "input.fullFile == 'range'",
...
tabPanel_operationResults <- tabPanel(
  "Operation Results",
  value = "operation",
  br(),
  bsAlert("alert"),
  conditionalPanel(
    condition = "input.operationType == 'fqinfo' || 'srainfo' || 'related_acc'",
    dataTableOutput('operationResultsTable')
  ),
  conditionalPanel(
    condition = "input.operationType == 'eGraph'",
...
shinyUI(
  fluidPage(
    ...
    tabsetPanel(
      id = "tabSet",
      tabPanel_home,
      tabPanel_searchResults,
      tabPanel_operationResults
    )
  )
)
